...
$language = $data['reg_language'];
$currency = $data['reg_currency'];
...
if ($check->num_rows != 0) {
    ...
    $sql = "INSERT INTO users(username, full_name, email, password_hash, preferred_language, preferred_currency)
    VALUES('$username', '$fullname', '$email', '$password', '$language', '$currency')";
    if($conn->query($sql)){
